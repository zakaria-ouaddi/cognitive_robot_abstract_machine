...
def init_kitchen_interface(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        if "iai_apartment_kitchen_msgs" not in sys.modules:
            logger.warning(
                "Could not initialize the Apartment kitchen interface since the iai_apartment_kitchen_msgs are not imported"
            )
            return
        ...
        if "/blum_control_service_server" in get_node_names():
            ...
            logger.warning(
                "Apartment kitchen is not running, could not initialize Apartment kitchen interface"
            )
            return
        authenticate_user()
...
def authenticate_user():
    ...
    try:
        ...
        resp1 = kitchen_service("authenticate_chk", "coraplex")
        if resp1.resp == "FALSE":
            logger.info("Please press the cyan button on the kitchen gateway")
            resp1 = kitchen_service("authenticate", "coraplex")
