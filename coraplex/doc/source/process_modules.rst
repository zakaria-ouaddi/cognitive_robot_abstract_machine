Process Modules in CoraPlex
=========================

Process modules are CoraPlex’s bridge from symbolic intentions to concrete robot motions. They encapsulate the low‑level
...
The base class ``coraplex.process_module.ProcessModule`` provides per‑module locking and an optional execution delay to make
...
of concrete modules is delegated to ``coraplex.process_module.ProcessModuleManager`` and robot‑specific managers derived from
...
---------------------------------------------
CoraPlex switches execution context using the ``ExecutionType`` enum and light‑weight context helpers. Within a
``coraplex.process_module.simulated_robot`` context (or when decorating a function with ``@with_simulated_robot``), the
manager returns simulated implementations. Within a ``coraplex.process_module.real_robot`` context (or ``@with_real_robot``),
...
``coraplex.process_modules.default_process_modules``) registers itself under the robot name ``"default"`` and implements a
...
--------------------------------
Beyond purely simulated and purely real contexts, CoraPlex provides a ``SemiRealRobot`` context for hybrid scenarios.
...
implement the relevant factory methods. Register the manager by importing it from ``coraplex.process_modules`` so that
