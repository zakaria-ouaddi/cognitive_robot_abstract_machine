...
class CollisionWorldSynchronizer:
    ...
    def get_external_collision_symbol(self) -> List[cas.Symbol]:
        ...
        for link_name, max_idx in self.external_monitored_links.items():
            for idx in range(max_idx + 1):
                symbols.append(self.external_link_b_hash_symbol(link_name, idx))

                v = self.external_map_V_n_symbol(link_name, idx)
                symbols.extend([v.x.free_symbols()[0], v.y.free_symbols()[0], v.z.free_symbols()[0]])

                symbols.append(self.external_contact_distance_symbol(link_name, idx))

                p = self.external_new_a_P_pa_symbol(link_name, idx)
    ...
    def external_contact_distance_symbol(self,
        ...
        if link_b_name is None:
            ...
            return symbol_manager.register_symbol_provider(name=f'closest_point({link_name})[{idx}].contact_distance',
                                                           provider=provider)
        assert link_b_name is not None
        provider = lambda l1=link_name, l2=link_b_name: (
        ...
        return symbol_manager.register_symbol_provider(
            name=f'closest_point({link_name}, {link_b_name}).contact_distance',
            provider=provider)
    ...
    def external_link_b_hash_symbol(self,
        ...
        return symbol_manager.register_symbol_provider(
            name=f'closest_point({link_name}, {link_b_name}).link_b_hash',
            provider=provider
        )
